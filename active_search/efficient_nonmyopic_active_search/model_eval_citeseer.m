...
n_ks                = numel(ks);
n_experiments       = 20;
paks                = nan(numel(combine_coefs) + 1, n_experiments, numel(ks));
aucs_roc            = nan(numel(combine_coefs) + 1, n_experiments);
aucs_pr             = nan(numel(combine_coefs) + 1, n_experiments);
weight_L            = 0.1;
...
weights_L       = weight_L * (weights + 2 / weight_L * speye(problem.num_points));
...
for exp = 1:n_experiments
    ...
    for c_id = 1:n_coefs
        ...
        for k_id = 1:numel(ks)
            paks(c_id, exp, k_id) = get_pak(labels_H(test_ind), ...
                probs, 1, ks(k_id));
        end
        
        [auc_roc, auc_pr]      = get_auc(labels_H(test_ind), probs, 1);
        aucs_roc(c_id, exp)    = auc_roc;
        aucs_pr(c_id, exp)     = auc_pr;
    end
    % probs = mf_knn_model(problem, train_ind_L, observed_labels_L, ...
    %     train_ind_H, observed_labels_H, test_ind, weights, [0.1, 1], 2);
    ...
    for k_id = 1:numel(ks)
        paks(n_coefs + 1, exp, k_id) = ...
            get_pak(labels_H(test_ind), probs, 1, ks(k_id));
        
        paks(n_coefs + 2, exp, k_id) = ...
            get_pak(labels_H(test_ind), probs_H, 1, ks(k_id));
    end
    
    [auc_roc, auc_pr] = get_auc(labels_H(test_ind), probs, 1);
    aucs_roc(n_coefs + 1, exp)  = auc_roc;
    aucs_pr(n_coefs + 1, exp)   = auc_pr;
    
    [auc_roc, auc_pr] = get_auc(labels_H(test_ind), probs_H, 1);
    aucs_roc(n_coefs + 2, exp)   = auc_roc;
    aucs_pr(n_coefs + 2, exp)    = auc_pr;
end
